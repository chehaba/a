...
**Purpose:** Define inputs, skills, required intermediate artifacts, outputs, and schemas for the messaging/targeting agent. Automation-ready for Cursor IDE.
...
| Asset | Path |
|-------|------|
| Market tables | cloud-migration-modernization/Market Analysis Tables.md |
| Targeting strategy | cloud-migration-modernization/Targeting Strategy.md |
| Messaging framework | outbound/Messaging Framework.md |
| Enterprise motion | outbound/Enterprise Legacy On-Prem Sales Motion.md |
| Opportunities | data/opportunities/Oops.csv |
| Inbound leads | data/Inbound/Inbound.xlsx |
| Austin registry | data/austin-chamber/directory.json |
| Company target matrix | reports/Company Type Target Matrix.md |
| Open leads | reports/Open Leads Best to Target.md |
...
|-------|---------|--------|
| Detect tech stack | `python3 scripts/detect_tech_stack.py "URL"` | Technologies from HTML (evidence-based) |
| Scrape Austin registry | `python3 scripts/scrape_austin_chamber.py --categories 328 323` | data/austin-chamber/directory.json |
| Export leads to doc | `python3 scripts/export_austin_leads.py [--refresh]` | reports/Austin Registry Leads.md |
| Enrich leads with tech | `python3 scripts/enrich_leads_with_tech.py` | reports/Austin Leads With Tech Stack.md |

**Full skill reference:** agent/SKILLS.md
...
### Inbound.xlsx (Input Data)

```
Description, Source Campaign, No. of Employees, Email, [other columns]
```

**Description:** Free text (parsed for Ask, Title, Industry, Pain). **Email:** Enables domain-based company type inference.
...
|------|--------|----------|
| 1 | Read assets (AGENTS.md table) | — |
| 2 | Extract (companies, personas, triggers, pain) | Artifact 1 |
| 3 | Cross-reference, filter Microsoft | Artifact 2 |
| 4 | Rank, build messages | Output 1, 2, 3 |

**When Austin leads needed:** Run `export_austin_leads.py` (or `enrich_leads_with_tech.py` for tech detection). Output written to `reports/`.

**When tech stack needed:** Run `detect_tech_stack.py` on URL. Do not guess.
...
- [ ] Tech stack: evidence from HTML, not guessed
